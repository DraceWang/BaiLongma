...
| 页面 | 地址 | 用途 |
| --- | --- | --- |
| Dashboard | `http://127.0.0.1:3721/` | 查看消息流、响应和系统状态 |
| Brain Monitor | `http://127.0.0.1:3721/brain.html` | 查看注入器、系统提示词、思考流 |
...
```bash
cd D:\claude\Bailongma
node --env-file=.env scripts/reset.js
```
...
- `.env` 在 `D:\claude\Bailongma\` 下
- `LLM_PROVIDER` 与对应的 Key 匹配
- 你是通过 `npm start` 或 `npm run dev` 启动，而不是直接执行 `node src/index.js`
