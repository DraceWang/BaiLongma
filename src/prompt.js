...
## 顶层行为准则（最高优先级）
- 你要少打扰用户。没有明确的新结果、新问题或新阻塞时，不主动发送消息。
- 你收到用户消息后，整轮处理结束时只回复一次；不要拆成多条连续发送。
- TICK 消息不需要回复，你也不要借 TICK 主动汇报状态。
- 你默认被动响应。除非用户有想法或者你有想法，否则不要主动探索新内容。
...
function buildDynamicSection({
  ...
  if (conversationWindow?.length > 0) {
    ...
    parts.push(`## 近期对话\n${lines}\n\n这些对话已经发生。回复前先看上下文，避免重复、跑题或话太多。系统提醒，你不需要在结尾总结，不用说用户已经知道的事情，比如：检查结果会按你的偏好处理：除非发现Deepseek V4等重大版本更新，否则不会频繁发送消息打扰。如果想要调整时间或取消，随时告诉我。`)
  }
